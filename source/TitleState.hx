...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		// Additional support for mods, basically.
		if (titleJSON.backgroundSprite != null && titleJSON.backgroundSprite.length > 0 && titleJSON.backgroundSprite != "none"){
			bg.loadGraphic(Paths.image(titleJSON.backgroundSprite));
		}
		else if (titleJSON.backgroundSprite == "none"){
			bg.makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
		}
		else
		{
			bg.loadGraphic(Paths.image('startscreen/palacetitle'));
		...
		adaleotitleborder = new FlxSprite();
		// Additional support for mods, basically.
		if (titleJSON.backgroundSprite != null && titleJSON.backgroundSprite.length > 0){
			trace('SWITCHING TITLE SCREENS!?');
			adaleotitleborder.makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK); //set to black
			adaleotitleborder.y = 2000; //doesn't need used, so we can throw it out
		}
		else
		{
			adaleotitleborder.loadGraphic(Paths.image('startscreen/border'));
			adaleotitleborder.scrollFactor.set(0, 0);
			adaleotitleborder.updateHitbox();
			adaleotitleborder.screenCenter();
			adaleotitleborder.blend = MULTIPLY;
			adaleotitleborder.antialiasing = true;
		}
		add(adaleotitleborder);
		...
		logoBl.updateHitbox();
		if (path == "assets/images/logoBumpin.png"){
		...
		swagShader = new ColorSwap();
			...
			sariaMyBeloved = new FlxSprite(650, 14);
			
		#if (desktop && MODS_ALLOWED)
		var path = "mods/" + Paths.currentModDirectory + "/images/Saria title screen.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "mods/images/Saria title screen.png";
		//trace(path, FileSystem.exists(path));
		}
		if (!FileSystem.exists(path)){
			path = "assets/images/Saria title screen.png";
		//trace(path, FileSystem.exists(path));
		}
		sariaMyBeloved.frames = FlxAtlasFrames.fromSparrow(BitmapData.fromFile(path),File.getContent(StringTools.replace(path,".png",".xml")));
		#else
		
		sariaMyBeloved.frames = Paths.getSparrowAtlas('Saria title screen');
		#end
			sariaMyBeloved.animation.addByPrefix('dance', 'Saria title', 24, false);
			sariaMyBeloved.animation.play('dance');
			sariaMyBeloved.updateHitbox();
	
		sariaMyBeloved.antialiasing = ClientPrefs.globalAntialiasing;
	...
	override function beatHit()
	{
		...
		if (sariaMyBeloved != null)
			sariaMyBeloved.animation.play('dance', true);
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// credTextShit.visible = true;
				case 13:
					addMoreText('Adaleo Showdown');
				// credTextShit.text += '\nNight';
				case 14:
					addMoreText('By Haveorc'); // credTextShit.text += '\nFunkin';
