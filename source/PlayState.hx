...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Are you even trying?', 0.2], //From 0% to 19%
		['Shit', 0.4], //From 20% to 39%
		['Bad', 0.5], //From 40% to 49%
		['Subpar', 0.6], //From 50% to 59%
		['Decent', 0.69], //From 60% to 68%
		['Nice!!', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Amazing!', 0.9], //From 80% to 89%
		['Funkin!', 1], //From 90% to 99%
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'tutorial' | 'adaleo' | 'eternal' | 'twin-spark' | 'guns': //Imagine nerfing Twin Spark OMEGALUL
					curStage = 'entrance'; //SARIA MY BELOVED
				default:
					curStage = 'stage';
			}
		}
		...
		switch (curStage)
		{
			case 'entrance': //Tutorial + Week 1 (Adaleo Showdown)
				var bg:BGSprite = new BGSprite('palaceentrance/PEntrance', -550, -85, 0.93, 0.93);
				add(bg);
				PEntLight = new BGSprite('palaceentrance/PEntLight', -550, -85, 0.93, 0.93);
				PEntLight.blend = OVERLAY;
				
			case 'stage': //Week 1 (original game)
		...
		kyran = new FlxSprite(450, 560);
		kyran.frames = Paths.getSparrowAtlas('kyran_leaning');
		kyran.animation.addByPrefix('dance', 'Kyran dance', 24, false);
		kyran.animation.play('dance');
		kyran.updateHitbox();
		kyran.antialiasing = ClientPrefs.globalAntialiasing;
		...
		switch(songName)
		{
			case 'eternal':
				...
				{
					trace ('And suddenly, Kyran arrives.');
					add(kyran);
				}
		}
		add(dadGroup);
	...
	override public function update(elapsed:Float)
	{
		...
		var defaultDadY:Float = dad.y;
		if (dad.curCharacter == "saria_fly") //Saria flying - up and down.
		{
			dad.y = defaultDadY + Math.sin((Conductor.songPosition / 2000) * (Conductor.bpm / 60) * 1) / 2; //up and down she goes, i guess
		}
		if (dad.curCharacter != "saria_fly") //Stop fly
		{
			dad.y = defaultDadY;
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Blammed Lights': // OMG IS THAT BLAMMED LIGHTS!?!?!?!? - Haveorc
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (note.noteType == 'Star Note') { //Star note shit
			if (health > 0.08) //It no longer blue balls you!?
			{
				health -= 0.08;
			}
		}
		...
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					case 'Hurt Note' | 'Star Note': //Share between the two
